#!/bin/bash
# test for Btrfs Filesystem with Sleuthkit from Shujian Yang.
...
oneTimeSetUp() {
	...
	tskMachineName="sleuthkit_sy"
	srcScript="btrfs2_generate.sh"
	tskScript="btrfs2_analyze.sh"
#<<COMMENT
	# generate VM Names
	...
	# generate .vmdk
	vmdkFilePath="${baseDir}/data/btrfs2.vmdk"
...
#COMMENT
	...
	srcJson=$(jq . < ${baseDir}/data/btrfs2src.json | tr '[:upper:]' '[:lower:]' | sed -e 's/ null / "" /g')
	tskJson=$(jq . < ${baseDir}/data/btrfs2tsk.json | tr '[:upper:]' '[:lower:]' | sed -e 's/ null / "" /g')

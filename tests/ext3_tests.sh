...
testPARTITION(){
	src=$(jq '.partition.part_type' <<< "$srcJson" | tr -d '"')
	tsk=$(jq '.partition.part_type' <<< "$tskJson" | tr -d '"')
...
testFILESYSTEM(){
	src=$(jq '.fs.type' <<< "$srcJson"| tr -d '"')
	tsk=$(jq '.fs.type' <<< "$tskJson"| tr -d '"')
	...
	src=$(jq '.fs.name' <<< "$srcJson"| tr -d '"')
	tsk=$(jq '.fs.name' <<< "$tskJson"| tr -d '"')
	...
	src=$(jq '.fs.id' <<< "$srcJson"| tr -d '"')
	tsk=$(jq '.fs.id' <<< "$tskJson"| tr -d '"')
	...
	src=$(jq '.fs.os' <<< "$srcJson"| tr -d '"')
	tsk=$(jq '.fs.os' <<< "$tskJson"| tr -d '"')
...
oneTimeSetUp() {
	...
	srcScript="ext3_generate.sh"
	tskScript="ext3_analyze.sh"
#<<COMMENT
	# generate VM Names
	...
	# generate .vmdk
	vmdkFilePath="${baseDir}/data/ext3.vmdk"
...
#COMMENT
	...
	srcJson=$(jq . < ${baseDir}/data/ext3src.json | tr '[:upper:]' '[:lower:]' | sed -e 's/ null / "" /g')
	tskJson=$(jq . < ${baseDir}/data/ext3tsk.json | tr '[:upper:]' '[:lower:]' | sed -e 's/ null / "" /g')
